...
def smb_df_generation():
    ...
    return list_of_dfs, dfs


def radius_df_generation():
    df_radius = pd.read_csv(radius_csv[0], header=None, names=columns_radius)
    df2_radius = df_radius[df_radius['Message'].str.contains(' searching for user ')]
    return df_radius, df2_radius
...
def smb_log_line_extraction(start_time, end_time, netbios_name):
    for name in dfs:
        ...
        if netbios_name in name:
            ...
            df_samba[name].set_index(['Time'], inplace=True)
            print(df_samba[name])
        return df_samba[name].loc[start_time:end_time]
...
def user_session_ids():
    ...
    for i in df2_radius.index:
        line1 = df2_radius['Thread/RequestNo./SessionID'][i]
        line2 = df2_radius['Message'][i]
...
def filterSessionId(sessionID):
    df14 = df_policy[df_policy['Hmmm...'].str.contains(sessionID)]
    #     print(df3)
    return df14
# unique session ids:
def unique_sessions():
...
print('csv creation completed in: ', end1 - start)
with concurrent.futures.ThreadPoolExecutor() as executor:
    radius_files, radius_config,radius_config_csv, radius_csv, radius_folder = executor.submit(radius_folder_files, path).result()
    policy_files, policy_files_csv, policy_folder = executor.submit(policy_folder_files, path).result()
    samba_files, samba_files_csv, samba_folders = executor.submit(samba_folder_files, path).result()
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    radius_result, policy_result, postgres_result, systemLoadMonitor_result, smb_result = [executor.submit(func) for func in [radius_df_generation, policy_df_generation, postgres_df_generation, system_load_monitor_df_generation, smb_df_generation]]
    df_radius = radius_result.result()[0]
    df2_radius = radius_result.result()[1]
    ...
    df_samba = smb_result.result()[0]
    dfs = smb_result.result()[1]
